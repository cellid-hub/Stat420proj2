...
# sqrt(n)(xBar^2 - mu^2) -> N(0, 1/I(mu^2)) = N(0, 1/(1/4*sigma^2*mu^2)) = N(0, 4) is limiting dist
for (n in ns){
  theta <- replicate(5000,mean(rnorm(n, mean=mu, sd=sd))^2)
  hist(sqrt(n)*(theta-mu^2), freq=FALSE, main=list(paste("n=",n," : N(",round(mean(sqrt(n)*(theta - mu^2)), 5),",",round(var(sqrt(n)*(theta - mu^2)), 5),"), expected: N(", 0,",",4,")")))
